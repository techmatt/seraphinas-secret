 ...
 *
 * Morgana is the same machinery with a different child in it, which is why she is
 ...
 * get the next thing, wrap for ever — is asked of Morgana after Sneak's job has
...
test('people can be walked through, pressed, and answer over their own heads', async ({ page }) => {
  ...
  const morgana = npc(start, 'morgana');
  const { tile, cols, blocked } = start.world;
  expect(
    blocked[Math.floor(morgana.y / tile) * cols + Math.floor(morgana.x / tile)],
  ...
  expect(
    Math.hypot(inside.player.x - morgana.x, inside.player.y - morgana.y),
  ...
  expect(her.voice.lineId, 'the first line of the job she is offering').toBe('morgana_story_offer');
  expect(her.voice.bubble.speaker, 'her balloon, over her').toBe('morgana');
  expect(Math.abs(her.voice.bubble.x - morgana.x)).toBeLessThan(40);
  ...
  // Pressing green at Morgana is the next thing she has to say, and it wraps for
  // ever: there is no end to a conversation and nothing in one to get wrong.
  await standNear(page, 'morgana', { y: 72 });
  await tap(page, 'KeyZ');
  expect((await readHooks(page)).voice.lineId, 'her own first line, at last').toBe(morgana.lines[0]);
  await tap(page, 'KeyZ');
  expect((await readHooks(page)).voice.lineId, 'pressing again says her second').toBe(
    morgana.lines[1],
  );
  expect(morgana.lines[1], 'which is a different line').not.toBe(morgana.lines[0]);
  await tap(page, 'KeyZ');
  expect((await readHooks(page)).voice.lineId, 'and then it wraps').toBe(morgana.lines[0]);
